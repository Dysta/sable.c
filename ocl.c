...
cl_mem tex_buffer, cur_buffer, next_buffer, diff_counter;
...
void ocl_alloc_buffers (void)
{
  ...
  diff_counter = clCreateBuffer(context, CL_MEM_READ_WRITE, 
                                sizeof(int), NULL, NULL);
  if (!diff_counter)
    exit_with_error ("Failed to allocate diff counter buffer");
}
...
unsigned ocl_invoke_kernel_generic (unsigned nb_iter)
{
  ...
  int diff; // used to detect diff

  for (unsigned it = 1; it <= nb_iter; it++) {

    diff = 0;
    check(
      clEnqueueWriteBuffer(queue, diff_counter, CL_TRUE, 0,
			      sizeof(int), &diff, 0, NULL, NULL),
            "Failed to write to diff_counter"
    );
    ...
    err |= clSetKernelArg (compute_kernel, 2, sizeof (cl_mem), &diff_counter);
    ...
    check(
      clEnqueueReadBuffer(queue, diff_counter, CL_TRUE, 0,
			      sizeof(int), &diff, 0, NULL, NULL),
            "Failed to read in diff_counter"
    );

    if (diff == 0) {
    //   printf("no diff at it %d, stop\n", it);
      return it;
    }
