...
// {
//     TABLE       = calloc(DIM * DIM, sizeof(uint32_t));
//     stable_tile = calloc(TILEX * TILEY, sizeof(unsigned));
...
//     ocl_stable_tile = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(unsigned) * TILEX * TILEY, NULL, NULL);
//     if (!stable_tile)
//         exit_with_error("Failed to allocate stable tile buffer");

//     check(
//         clEnqueueWriteBuffer(queue, ocl_stable_tile, CL_TRUE, 0,
//                                 sizeof(unsigned) * TILEX * TILEY, 
